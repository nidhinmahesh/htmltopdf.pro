<script lang="ts">
	...
	function handleFiles(newFiles: File[]) {
		const images = newFiles.filter((f) => f.type === 'image/png');
	...
	async function convert() {
		...
		try {
			...
			for (const { file } of files) {
				...
				const image = await pdf.embedPng(new Uint8Array(bytes));
...
<ToolPageLayout {tool}>
	{#snippet children()}
		<FileDropZone accept=".png" label="Drop PNG images here or click to browse" onfiles={handleFiles} />
			...
			<div class="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-4">
				{#each files as f, i}
					<div class="group relative overflow-hidden rounded-lg border border-neutral-200 dark:border-neutral-800">
						<img src={f.preview} alt={f.name} class="aspect-[3/4] w-full object-cover" />
						<div
	...
	{#snippet article()}
		<h2 class="mb-4 text-lg font-semibold text-neutral-800 dark:text-neutral-200">
			Free PNG to PDF converter — no upload required
		</h2>
		<p class="mb-3">
			Convert PNG images to PDF entirely in your browser. PNG format preserves transparency and
			crisp edges. Add multiple images and combine them into a single PDF document. Your images
			never leave your device.
		</p>

		<h3 class="mb-2 mt-6 font-semibold text-neutral-700 dark:text-neutral-300">How to convert PNGs to PDF</h3>
		<ol class="mb-3 list-decimal space-y-1 pl-5">
			<li>Drop your PNG images into the upload area</li>
			<li>Reorder images using the arrow buttons</li>
