<script lang="ts">
	...
	function downloadPage(page: { url: string; index: number }) {
		...
		a.href = page.url;
		a.download = `page-${page.index}.png`;
		a.click();
	}
...
<ToolPageLayout {tool}>
		...
		{#if pages.length > 0}
			...
			<div class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
				{#each pages as page}
					...
					>
						<img src={page.url} alt="Page {page.index}" class="aspect-[3/4] w-full object-cover" />
	...
	{#snippet article()}
		<h2 class="mb-4 text-lg font-semibold text-neutral-800 dark:text-neutral-200">
			Free PDF to PNG converter — no upload required
		</h2>
		<p class="mb-3">
			Convert PDF pages to high-quality PNG images with transparency support, entirely in your
			browser. PNG format preserves crisp text and sharp edges better than JPG. Your files never
			leave your device.
		</p>
		...
		<ol class="mb-3 list-decimal space-y-1 pl-5">
			...
			<li>Click "Convert to PNG" — each page renders as an image</li>
